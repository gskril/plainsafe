...
- **Code:** Claude Code writes the implementation step by step in the order of SPEC §16, with the human author reviewing and steering. So far that covers:
  ...
  - P1 so far: MultiSend batches with per-call decoding and safety rules, Swap through Uniswap (on-chain quotes across v3 and v4, the TWAP check, Universal Router 2.2.0 encoding checked against the router's source, and the fresh-quote panel on review), `approveHash`, one-click cancel, queue simulation, on-chain history (the backwards scanner, its Web Worker and the History view), the IPFS release tooling (`scripts/compute-cid.ts`, `RELEASE.md`, the release workflow) and the ENS contenthash helper
