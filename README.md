...
- **Code:** Claude Code writes the implementation step by step in the order of SPEC §16, with the human author reviewing and steering. So far that covers:
  ...
  - P1 so far: MultiSend batches with per-call decoding and safety rules, `approveHash`, one-click cancel, queue simulation, and on-chain history (the backwards scanner, its Web Worker and the History view)
